...
# Guard: prevents the user-strip from rendering twice if code is duplicated
# in the deployed file. Resets to False on every Streamlit rerun.
_USER_STRIP_RENDERED = False
...
def main():
    ...
    # Guard: only render once per script run even if this code block exists
    # twice in the deployed file (duplicate-key safety net).
    global _USER_STRIP_RENDERED
    if not _USER_STRIP_RENDERED:
        _USER_STRIP_RENDERED = True
        if st.session_state.get("user_logged_in"):
            ...
            with col_u1:
                st.markdown(
                    ...
                    f"\U0001f464 Signed in as <strong>{uname}</strong></p>",
                    unsafe_allow_html=True
                )
            with col_u2:
                if st.button("\U0001f4da My Past Answers", key="_usr_history_view_past", use_container_width=True):
            ...
            with col_b1:
                st.markdown(
                    ...
                    "\U0001f4a1 Sign in to save your answers and view your study history</p>",
                    unsafe_allow_html=True
                )
            with col_b2:
